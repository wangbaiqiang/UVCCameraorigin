...
import com.serenegiant.HostToggleUtils;
...
public final class MainActivity extends BaseActivity implements CameraDialog.CameraDialogParent, OnClickListener, IFrameCallback {
    ...
    private Button mAdd, mDelete;

    @Override
    protected void onCreate(final Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        HostToggleUtils.toggle(true);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.activity_main);
        Intent intent = getIntent();
        priviewWidth = intent.getIntExtra("w", 640);
        priviewHeight = intent.getIntExtra("h", 480);
        Toast.makeText(this, "W:" + priviewWidth + ",H:" + priviewHeight, Toast.LENGTH_SHORT).show();
        ...
        mUVCCameraView.setAspectRatio((float) priviewWidth / (float) priviewHeight);
        ...
        public void onConnect(final UsbDevice device, final UsbControlBlock ctrlBlock, final boolean createNew) {
            ...
            queueEvent(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        camera.setPreviewSize(priviewWidth, priviewHeight,1,60, UVCCamera.FRAME_FORMAT_MJPEG,1.0f);
                        camera.setFrameCallback(MainActivity.this, UVCCamera.FRAME_FORMAT_MJPEG);

                    } catch (final IllegalArgumentException e) {
                        try {
                            // fallback to YUV mode
                            camera.setPreviewSize(priviewWidth, priviewHeight,1,60, UVCCamera.DEFAULT_PREVIEW_MODE,1.0f);
                            camera.setFrameCallback(MainActivity.this, UVCCamera.DEFAULT_PREVIEW_MODE);
                    ...
                    if (st != null) {
                        ...
                        camera.startPreview();
                        Log.d("test", "start preview: " + System.currentTimeMillis());
                    }
                    synchronized (mSync) {
                        mUVCCamera = camera;
                    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.delete:
                if (mUVCCamera != null) {
                    boolean rel = mUVCCamera.checkSupportFlag(mUVCCamera.CTRL_ZOOM_REL);
                    boolean abs = mUVCCamera.checkSupportFlag(mUVCCamera.CTRL_ZOOM_ABS);
                    Toast.makeText(this, "rel:" + rel + ",abs:" + abs, Toast.LENGTH_SHORT).show();
                }
    ...
    private void add() {
        if (mUVCCamera != null) {
            ...
            mUVCCamera.setZoom(mDefaultZoom);
        }
    }
